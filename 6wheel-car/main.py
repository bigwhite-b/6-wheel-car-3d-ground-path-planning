from car_robot_env import CarRobotEnv
...
def train_algo():
    ...
    env = CarRobotEnv()  # Instantiate the custom driving environment
    ...
    model_name = f"car_robot_{algo_name.lower()}_3d" # New model name for 3D observation
    ...
    checkpoint_callback = CheckpointCallback(save_freq=500000, save_path="./models", name_prefix=model_name)
...
def test_algo():
    ...
    # Note: If you haven't trained the _3d model yet, this will fail.
    # You need to run train_algo() first with the new observation space.
    try:
        model = model.load(model_name)
    except FileNotFoundError:
        print(f"Model {model_name} not found. Please train it first by running train_algo().")
        return
